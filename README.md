...
- **`plots.html`** — Violin / Density / Line plots (tabs switch chart type):
  ...
  - Region annotations on line plots with an auto-built legend.
...
- **`multipanel.html`** — compose multiple datasets into one page:
  - **Violin + Density Grid**: upload multiple CSV/Excel files (one file = one column); every 3 files start a new violin-row + density-row pair, sized to fit an A4 (or custom) page, with auto panel labels (A, B, C…) and a single shared 25–75%/IQR/Mean legend.
  - **Line Stack**: add any number of line plots stacked vertically on one page, each with its own data and region annotations; toggle "Same formatting for all" for one shared X/Y range & tick control, or uncheck it to set each plot's range, ticks, and height independently.
